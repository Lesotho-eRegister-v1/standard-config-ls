(SELECT  patientIdentifier , patientName, Age, Gender, Contact, 'Initiated' AS 'Program_Status'
	FROM
                ...
                from obs o
						...
						AND YEAR(o.value_datetime) = YEAR(CAST('#endDate#' AS DATE))
						 ...
						 LEFT JOIN person_attribute pa ON pa.person_id = patient.patient_id AND pa.person_attribute_type_id = 26 AND pa.voided = 0
...
UNION
(SELECT patientIdentifier , patientName , Age, Gender, Contact, 'Seen' AS 'Program_Status'
FROM (
...
(SELECT patientIdentifier , patientName , Age, Gender, Contact, 'MissedWithin28Days' AS 'Program_Status'
FROM
                 (select distinct patient.patient_id AS Id,
									   patient_identifier.identifier AS patientIdentifier,
									   concat(person_name.given_name, ' ', person_name.family_name) AS patientName,
...
(SELECT  patientIdentifier , patientName, Age, Gender, Contact, 'Seen_Previous' AS 'Program_Status'
	FROM
                (select distinct patient.patient_id AS Id,
									   patient_identifier.identifier AS patientIdentifier,
									   concat(person_name.given_name, ' ', person_name.family_name) AS patientName,
